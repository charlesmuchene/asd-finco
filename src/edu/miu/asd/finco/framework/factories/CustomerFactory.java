...
public class CustomerFactory implements AbstractCustomerFactory {
    ...
    @Override
    public ICustomer createCustomer(ICustomer.Type type, String name, String street, String city, String zip, String state, String email, int nOfEmployees, LocalDate birthDate) {

        switch (type) {
            case ORGANIZATION:
                return new Organization(name, street, city, zip, state, email, nOfEmployees);
            case PERSON:
                return new Person(name, street, city, zip, state, email, birthDate);
        }
